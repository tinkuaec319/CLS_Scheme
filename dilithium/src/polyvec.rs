...
pub fn polyvecl_chknorm_big(v: &Polyvecl, bound: i32) -> u8 {
  for i in 0..L {
    if poly_chknorm_big(&v.vec[i], bound) > 0 {
      return 1;
    }
  }
  return 0;
...
pub fn polyveck_make_hint_scaled(
    ...
    let mut s = 0;
    for i in 0..K {
        s += poly_make_hint_scaled(&mut hint.vec[i], &z.vec[i], &y.vec[i]);
    }
...
pub fn polyveck_use_hint_scaled(
    ...
    for i in 0..K {
        poly_use_hint_scaled(&mut high.vec[i], &hint.vec[i], &y.vec[i]);
    }
}
...
pub fn polyveck_decompose_scaled(high: &mut Polyveck, low: &mut Polyveck,
    y: &Polyveck) {
    for i in 0..K {
        poly_decompose_scaled(&mut high.vec[i], &mut low.vec[i], &y.vec[i]);
    }
}
