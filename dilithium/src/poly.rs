...
pub fn poly_chknorm_big(a: &Poly, b: i32) -> u8 {
  // It is ok to leak which coefficient violates the bound since
  // the probability for each coefficient is independent of secret
  // data but we must not leak the sign of the centralized representative.
  let mut t;
...
/// coefficients in {-1,1} using the output stream of
/// SHAKE256(seed).
pub fn poly_challenge_nonced(c: &mut Poly, seed: &[u8], nonce: u16) {
  let mut _signs = 0u64;
  let mut buf = [0u8; SHAKE256_RATE];
  let mut state = KeccakState::default(); //shake256_init

  // stream256_init(&mut state, &seed[..SEEDBYTES], nonce);
  shake256_absorb(&mut state, seed, SEEDBYTES);
  shake256_absorb(&mut state, &nonce.to_ne_bytes(), 2);
  shake256_finalize(&mut state);
  shake256_squeezeblocks(&mut buf, 1, &mut state);
